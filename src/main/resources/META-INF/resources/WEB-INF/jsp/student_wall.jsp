				...
				<div class="row">
					<div class="[ col-xs-12 col-sm-offset-1 col-sm-10 col-md-10 ]">
						<div class="[ panel panel-default ] panel-google-plus"
							...
							<div class="dropdown">
								<c:choose>
									<c:when test="${idea.status == 0}">
									</c:when>
									<c:when test="${idea.status == 3}">
									</c:when>
									<c:when test="${idea.status == 4}">
									</c:when>
									<c:when test="${idea.status == 5}">
									</c:when>
									<c:otherwise>
										<c:choose>
											<c:when
												...
												<ul class="dropdown-menu" role="menu">
													<li role="presentation"><a role="menuitem"
														tabindex="-1" class="editIdea"
														onclick="return editIdea(${idea.id});">Edit</a></li>
													<li role="presentation"><a role="menuitem"
														tabindex="-1"
								...
								<form id="edit_idea_form${idea.id}" method="post"
									...
									<div class="editIdeaInfo${idea.id}">
										...
										<textarea class="form-control" name="content"
											style="width: 100%" rows="4" required>${idea.content}</textarea>
							...
							<div class="panel-footer">
								<form id="like_form${idea.id}" method="post" action="/like/like">
									<input type="hidden" id="idea_id${idea.id}" name="idea"
										value=""> <input type="hidden" name="like"
										id="like${idea.id}" value="">
								</form>
